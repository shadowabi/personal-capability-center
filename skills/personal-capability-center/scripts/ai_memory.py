"""
Personal Capability Center System - PostgreSQL + pgvector

一个个人能力中心系统，支持向量相似度搜索、多维筛选和LangChain集成。
"""
...
class AIMemory:
    ...
    def add_conversation(
        ...
        注意:
            - 当 format_type='markdown' 时，建议使用标准的Markdown语法：
              ...
              * 列表：- 项目 或 1. 项目
              * 分隔符：---
            - 前端（如Personal Capability Center Dashboard）会自动渲染Markdown格式
        """
    ...
    def search_similar(
        self, query_vector: List[float], limit: int = 5, min_importance: int = 0
    ) -> List[Tuple]:
        """向量相似度搜索
    ...
    def get_by_tags(
        self, tags: List[str], limit: int = 10, match_all: bool = False
    ) -> List[Tuple]:
        """按标签搜索
    ...
    def get_by_importance(
        self, min_importance: int = 8, limit: int = 10
    ) -> List[Tuple]:
        """按重要性筛选
    ...
    def search_by_keyword(
        self, keyword: str, search_in_details: bool = True, limit: int = 10
    ) -> List[Tuple]:
        """关键词搜索
    ...
    def update_importance(self, conversation_id: int, new_importance: int) -> bool:
        """更新对话重要性
    ...
    def get_recent(self, days: int = 7, limit: int = 10) -> List[Tuple]:
        """获取最近的对话
